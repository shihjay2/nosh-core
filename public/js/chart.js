$(document).ready(function() {
	...
	$("#connect_patient_centric_dialog").dialog({ 
		...
		open: function (event, ui) {
			$.ajax({
				type: "POST",
				url: "ajaxchart/get-patient-centric",
				success: function(data){
					$("#connect_patient_centric_url").val(data);
				}
			});
		},
		buttons: {
			'Save': function() {
				var bValid = true;
				$("#connect_patient_centric_form").find("[required]").each(function() {
					var input_id = $(this).attr('id');
					var id1 = $("#" + input_id); 
					var text = $("label[for='" + input_id + "']").html();
					bValid = bValid && checkEmpty(id1, text);
					if (input_id == 'connect_patient_centric_url') {
				...
				if (bValid) {
					var str = $("#connect_patient_centric_form").serialize();
					if(str){
						$.ajax({
							type: "POST",
							url: "ajaxchart/connect-patient-nosh",
							data: str,
							dataType: 'json',
							success: function(data){
								if (data.status == 'y') {
									$.jGrowl(data.message);
									$("#connect_patient_centric_form").clearForm();
									$("#connect_patient_centric_dialog").dialog('close');
									$("#connect_patient_centric_status").html('Connected');
								} else {
									$.jGrowl(data.message);
								}
							}
	...
	$("#connect_patient_nosh").click(function() {
		$("#connect_patient_centric_dialog").dialog('open');
	});
